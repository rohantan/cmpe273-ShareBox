...
//		alert("inside onload");
		$.ajax({
			type: "GET",
			async:false,
			url: url+"/viewsharedbyme/" +emailid,
			success: function(msg){
				var obj = jQuery.parseJSON( ''+ msg +'' );
				var html1= '';
				var hrefViewHere='';
...
//						alert(hrefViewHere);
					...
					html1+='<tr class="ui-widget-content" id='+obj.sharedfiles[i].filename+'><td >'+obj.sharedfiles[i].filename+'</td><td >'+obj.sharedfiles[i].filesize+'</td><td >'+obj.sharedfiles[i].receiver+'</td><td><button '+disablity+' '+onclickval+' id="'+hrefViewHere+'" '+classval+'>View</button></td></tr>';
					hrefDelete='';
					hrefDownload = '';
					disablity='';
				}
